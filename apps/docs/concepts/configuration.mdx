...
- `~/.director/config.json` - This is the main configuration file that contains the list of playbooks, their targets and their configuration.
...
Your configuration file is a JSON file that defines playbook connections (it's location is `~/.director/config.json`). Here's how to structure it:

```json
{
  // The list of available playbooks
  "playbooks": [
    {
      "id": "unique-playbook-name",
      "name": "Display Name for Playbook",
      "description": "What this playbook does (optional)",
      // The list of servers that this playbook aggregates
      "servers": [
        {
          "name": "stdio-server",
          "transport": {
