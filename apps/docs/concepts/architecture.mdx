...
Director is a service that sits between your AI agents and MCP servers, providing a **playbook-based architecture** for organizing and managing MCP capabilities. It's transparent to clients, requiring no additional tokens or configuration changes.
...
The `Gateway` implements the playbook pattern by:
- Aggregating multiple MCP servers into a single playbook endpoint
- Supporting all MCP transports (HTTP Streamable, Stdio, SSE)
- Providing tool filtering to preserve context
- Offering unified OAuth for centralized authentication
