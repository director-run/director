...
export async function trust(name: string) {
  ...
  echo(await $`ssh-keygen -R ${ip} && ssh-keyscan -H ${ip} >> ~/.ssh/known_hosts`);
}

export async function copyId(name: string, user: string = "admin", password: string = "admin") {
  const ip = await getIP(name);
...
export async function create(name: string, image: string = "ghcr.io/cirruslabs/ubuntu:latest"): Promise<VM | undefined> {
